...
import matplotlib
matplotlib.use("Agg")  # Force non-interactive backend for Replit
...
# Initialize toolbox globally
toolbox = base.Toolbox()
toolbox.register("attr_float", random.random)
toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_float, n=10) # type: ignore
...
STAGE_TOPICS = {
    ...
    # ... (keeping this concise, same as before)
    "Thinker Phase": ["Animal consciousness", "Philosophical animal rights", "Ethical dilemmas in ecology", "Everything in the universe"]
}
...
"""
    ...
    def load_knowledge(self):
        if os.path.exists("ai_knowledge.npy"):
            self.knowledge_graph = np.load("ai_knowledge.npy", allow_pickle=True).tolist()
            print("Loaded existing knowledge graph.")

    def save_knowledge(self):
        np.save("ai_knowledge.npy", self.knowledge_graph)
        print("Saved knowledge graph.")

    def add_node(self, data, connect_to=None):
    ...
    def _rebuild_kd_tree(self):
        if not self.knowledge_graph:
            self.kd_tree = None
            return
        points = np.array([hash(node.data) % 100 for node in self.knowledge_graph]).reshape(-1, 1)
        self.kd_tree = KDTree(points)

    def pull_from_internet(self, query, num_results=5):
        try:
    ...
    def update_progress(self, fitness: float = 0.0, iteration: int = 0):
        with self.lock:
    ...
    def visualize_evolution(self) -> BytesIO:
        with self.lock:
            ...
            node_colors = ['#00ff88' if 'NEW' in G.nodes[n].get('label', '') else '#ff3366' if 'PRUNED' in G.nodes[n].get('label', '') else '#00d4ff' for n in G.nodes]
            ...
            nx.draw_networkx_nodes(
                ...
                node_colors=list(node_colors),  # type: ignore[arg-type]
    ...
    def generate_chat_response(self, msg: str) -> str:
        try:
            ...
            self.self_heal(e, "generate_chat_response")
            return "Brain hiccup: recovering..."
    ...
    def _self_evolve(self):
        """Internal adaptive evolution mechanism."""
        print("[SELF-EVOLVE] Adjusting learning dynamics...")
        # Example adaptive mutation tweak:
        self.mutation_rate = min(0.5, self.mutation_rate * random.uniform(0.9, 1.1))
        ...
        toolbox.register("evaluate", lambda ind: (sum(ind[:5]) - sum(abs(i - t) for i, t in zip(ind, [0.5] * 20)),))
        ...
        time.sleep(1)
        print("Web UI → http://12.0.0.1:5000")
        ...
        for stage_idx in range(len(STAGES)):
            ...
            while iteration < stage["min_iterations"] * 2 and self.running:
                try:
                    ...
                    phase_metrics = PhaseTrainingAlgorithms.train_baby_steps(self, X_batch, y_batch, toolbox, pop_size)
...
if __name__ == "__main__":
    import threading
    threading.Thread(target=lambda: ai.train(username="AdMiN"), daemon=True).start()
