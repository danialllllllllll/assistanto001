...
class GeneticTrainer:
    ...
    def _mutate_network(self, network, mutation_rate=0.1):
        ...
        # Preserve core values BEFORE any mutations
        if hasattr(network, 'core_values_lock'):
            preserved_values = network.core_values_lock.copy()
        else:
        ...
        # network.structural_mutations = mutations_applied # This line seems to be missing from the original context or change snippet. Assuming it's not critical for the fix.

        # RESTORE core values after mutations (prevent corruption)
        if 'preserved_values' in locals():
            network.core_values_lock = preserved_values

        return network # Assuming network is mutated in place and returned.
    ...
    def _copy_network(self, network):
        ...
        # Re-initialize Adam optimizer for new network with CORRECT shapes
        new_network.adam_m = {}
        new_network.adam_v = {}
        new_network.adam_t = 0

        for i, layer in enumerate(new_network.layers):
            if hasattr(layer, 'weights'):
                new_network.adam_m[f'W{i}'] = np.zeros_like(layer.weights)
                new_network.adam_v[f'W{i}'] = np.zeros_like(layer.weights)
            if hasattr(layer, 'bias'):
                new_network.adam_m[f'b{i}'] = np.zeros_like(layer.bias)
                new_network.adam_v[f'b{i}'] = np.zeros_like(layer.bias)

        # Ensure core values lock is copied or initialized
        if hasattr(network, 'core_values_lock'):
            new_network.core_values_lock = network.core_values_lock.copy()
        else:
            from core.core_values_guard import CoreValuesGuard
            CoreValuesGuard.verify_network(network) # This might re-initialize if missing
            new_network.core_values_lock = network.core_values_lock.copy()
    ...
    def load_state(self):
        ...
        if os.path.exists(self.state_file):
            ...
            if latest_checkpoint:
                ...
                # Load weights, biases, and scales into the template
                # This assumes network_template is a ProgressiveNeuralNetwork instance
                ...
                # Update Adam states based on the loaded network template
                self.network_template.adam_m = {}
                self.network_template.adam_v = {}
                self.network_template.adam_t = 0
                for i, layer in enumerate(self.network_template.layers):
                    if hasattr(layer, 'weights'):
                        self.network_template.adam_m[f'W{i}'] = np.zeros_like(layer.weights)
                        self.network_template.adam_v[f'W{i}'] = np.zeros_like(layer.weights)
                    if hasattr(layer, 'bias'):
                        self.network_template.adam_m[f'b{i}'] = np.zeros_like(layer.bias)
                        self.network_template.adam_v[f'b{i}'] = np.zeros_like(layer.bias)


                # Create the best_network from the loaded template
